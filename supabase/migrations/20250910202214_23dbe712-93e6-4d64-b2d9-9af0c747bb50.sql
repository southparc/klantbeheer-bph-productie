-- Restore the sophisticated insurance logic that was lost
CREATE OR REPLACE FUNCTION public.full_client_v2(p_email text)
RETURNS TABLE(
  id uuid, supabase_auth_id uuid, first_name text, last_name text, email text, phone text, initials text, prefix text,
  gender text, birth_date date, age integer, country text, employment_type text, planning_status text, risk_profile text,
  gross_income double precision, net_monthly_income double precision, net_monthly_spending double precision,
  saving_balance double precision, investment_balance double precision, pension_income double precision,
  retirement_target_age integer, monthly_fixed_costs double precision, monthly_variable_costs double precision,
  consumer_credit_amount double precision, house_id integer, is_owner_occupied boolean, home_value double precision,
  mortgage_amount double precision, mortgage_remaining double precision, mortgage_interest_rate double precision,
  annuity_amount double precision, annuity_target_amount double precision, energy_label text, current_rent double precision,
  contract_id integer, dvo double precision, max_loan double precision, is_damage_client boolean, insurance_id integer,
  disability_percentage integer, death_risk_assurance_amount double precision, insurance_premiums_total double precision,
  financial_goal_id integer, financial_goal_description text, financial_goal_amount double precision, goal_priority text,
  liability_id integer, liability_total_amount double precision, investment_id integer, investment_current_value double precision,
  advisor_name text, advisor_email text, partner_gross_income double precision
)
LANGUAGE sql
STABLE
SECURITY DEFINER
